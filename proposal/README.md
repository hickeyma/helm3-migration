...
- When deleting v2 release , only the release is deleted and not the underlying kubernetes resources
...
## 3.2 Helm v3 SDK updates

The following changes are required in the Helm v3 SDK:

- When adding a release, be able to set the following to correspond to the v2 details:
  - `modifiedAt`
  - `status`
  - `creationTimestamp`
  - `description`
...
- Be able to use Go modules for building the tool as it allows [multiple major versions (helm 2 and 3)](https://github.com/golang/go/wiki/Modules#v2-modules-allow-multiple-major-versions-within-a-single-build) to be used. Need @mattfarina [Go dependency PR](https://github.com/helm/helm/pull/5498) to merge
